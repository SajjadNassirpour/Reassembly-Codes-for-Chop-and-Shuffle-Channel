...
for iter=1:iters
    ...
    ran_vec=break_points_nested(cut_rule,n,ceil(2*cut_rule));
    ...
    frag1 = first_frag_single_r(m,L,d_sec,breaks_points,X_seperated1,VT_section_length,parity,Main_r);
    frag_final=frag1;
    ...
    for ii=1:size(frag_final,1)
        ...
        if time_upper<limited_time
            ...
            frag = other_frag_single_r(m,L,d_sec,breaks_points,data_combined,X_seperated_test,VT_section_length,parity,fragment_combined,r_mat,length_combined,rep_frag,Main_r,limited_time);
            if ~isempty(frag)
...
if passed_time>=limited_time && done==0
    recover_rate(iter)=0;
end
